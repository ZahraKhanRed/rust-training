...
impl Robot {
    fn move_forward(&mut self, Grid(Location { x, y }): &Grid) -> Result<(), LostRobot> {
        self.location = match self.direction {
            ...
            Direction::East if self.location.x >= 1 => Location {
                x: self.location.x + 1,
                y: self.location.y,
            },
...
mod tests {
    ...
    fn robot_moves_forward() {
        ...
        robot.move_forward(&Grid::new(Location { x: 5, y: 5 }));
    ...
    fn robot_follow_multiple_instructions_and_falls_off_the_grid() {
        ...
        let movement = robot.follows(&instructions, &Grid::new(Location { x: 5, y: 5 }));
        ...
        assert_eq!(movement.err().unwrap(), expected);
    }
}
